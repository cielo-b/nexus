...
export default function Home() {
  const [articles, setCareers] = useState<any[]>([]);
  ...
  const fetchCareersByCategory = async (page: number) => {
  ...
  useEffect(() => {
    fetchCareersByCategory(currentPage);
  }, [currentPage]);


  return isFetchingCareers ? (
    <Loader />
  ) : (

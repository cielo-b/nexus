...
import { fetchCaseById, fetchCases } from "@/sanity/queries/services";
...
export default function CasePage() {
  ...
  const [article, setCase] = useState<Case | null>(null);
  ...
  useEffect(() => {
    const fetchContent = async () => {
      if (!id) return;
      try {
        const articleData = await fetchCaseById(id)
        console.log(articleData)
        const relatedCasesData = await fetchCases(articleData.service._id).then((res) => {
    ...
    <main className="flex flex-col w-full h-full overflow-hidden overflow-x-hidden bg-gray-100">
      ...
      <div className="flex flex-col lg:flex-row gap-8  px-[10vw] ">
        {article ? (
          ...
          <div className="text-center text-gray-600 text-lg">No article found.</div>
        )}
        <div className="w-[35%]">
          <h3 className="text-2xl font-bold mb-4">Related Cases</h3>
          <div className="grid grid-cols-1 gap-6">
            {relatedCases.slice(0, 5).map((relatedCase) => (
              <Link
                href={`/blog/${relatedCase._id}`}
                key={relatedCase._id}
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 p-4">
                  <h3 className="text-2xl font-bold mb-2 text-white">{relatedCase.title}</h3>
                  <p className="text-lg text-white/70 line-clamp-1"><RichContent content={relatedCase.excerpt}></RichContent></p>
                </div>
              </Link>
            ))}
          </div>
